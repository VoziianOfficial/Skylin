...
(function () {
  ...
  function replaceHardcodedContent() {
    const replacements = config.dynamicReplacements || {};
    const pairs = [];

    addReplacementPairs(pairs, replacements.companyIds, config.companyId);
    addReplacementPairs(pairs, replacements.companyNames, config.companyName);
    addReplacementPairs(pairs, replacements.phones, config.phone);
    addReplacementPairs(pairs, replacements.phoneHrefs, config.phoneHref);
    addReplacementPairs(pairs, replacements.emails, config.email);
    addReplacementPairs(pairs, replacements.emailHrefs, `mailto:${config.email}`);
    addReplacementPairs(pairs, replacements.addresses, config.address.full);

    if (!pairs.length) return;

    pairs.sort((a, b) => b.from.length - a.from.length);
    document.title = replaceStringValues(document.title, pairs);
    ...
    textNodes.forEach((node) => {
      node.nodeValue = replaceStringValues(node.nodeValue, pairs);
    });
    ...
    document.querySelectorAll("*").forEach((element) => {
      attributesToUpdate.forEach((attribute) => {
        ...
        const nextValue = replaceStringValues(currentValue, pairs);
  ...
  function addReplacementPairs(pairs, oldValues, newValue) {
    if (!Array.isArray(oldValues) || !newValue) return;
  ...
  function replaceStringValues(value, pairs) {
